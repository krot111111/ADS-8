...
template <typename Type> class BST {
 private:
  ...
  void collectNodes(TreeNode *currentNode,
                    std::vector<std::pair<Type, int>> &result) const {
    if (currentNode == nullptr) {
      return;
    }
 ...
 public:
  ...
  void insert(const Type &val) { treeRoot = insertNode(treeRoot, val); }

  int depth() const { return calculateHeight(treeRoot); }

  int search(const Type &val) const { return findCount(treeRoot, val); }

  int size() const { return totalElements; }

  bool empty() const { return treeRoot == nullptr; }
